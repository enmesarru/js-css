/*
    Created by Furkan KURUTAŞ, 24.10.2018
*/
...
const calculateAllPossiblePath = (queue, col, row, array, total, list) => {
    ...
    if (row != 0) {
        if (!isPrime(array[col + 1][row - 1])) { // Sola
            calculateAllPossiblePath(queue, col + 1, row - 1, array, total, list);
        }
    }
    if (!isPrime(array[col + 1][row])) { // Düz
...
const findGreatestNumber = (array) => {
    if (array.length != 0) {
        if (array.length == 1) {
            ...
            return array.reduce((previous, current) => { return current < previous ? previous : current });
        }
    }
}
...
if(!isPrime(numbers[0][0])) {
    ...
    console.log(findGreatestNumber(values))
}
